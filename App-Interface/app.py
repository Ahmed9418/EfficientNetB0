...
def load_tflite_model():
    ...
    if not os.path.exists(MODEL_PATH):
        st.error(f"""
        **Model Not Found:** Could not find the EfficientNetB0 file at `{MODEL_PATH}`. 
        Please ensure your `.tflite` model is correctly uploaded 
        in the `App-Interface` folder on GitHub.
        """)
        return None

    try:
        with st.spinner(f"Loading EfficientNetB0 from {MODEL_PATH}..."):
            interpreter = tf.lite.Interpreter(model_path=MODEL_PATH)
            interpreter.allocate_tensors()
            st.success("✅ EfficientNetB0 TFLite Model loaded successfully!")
        return interpreter
...
def preprocess_image(image_data, input_details):
    """
    Preprocesses the uploaded image to match the format the EfficientNetB0 TFLite model expects.
    """
    ...
    # Normalize pixel values. EfficientNet models usually use [0, 1] normalization 
    # when exported as TFLite from standard Keras.
...
def predict_and_display(interpreter, processed_image):
    ...
    # 1. Run Prediction
    with st.spinner('Running EfficientNetB0 Pathogen Analysis...'):
        # Set the tensor
...
def main():
    ...
    st.title("🌿 Plant Pathogen Identifier (EfficientNetB0)")
    st.markdown(
        """
